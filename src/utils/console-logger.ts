...
export const print = (k:LogTypes,...args:any[]):boolean => {
  const isProd = Utils.isProd();
  const okErrWarn = ["ok","error","warn"].includes(k);
  ...
  switch(true){
    case isProd && !okErrWarn:break;
    case k == "here":logHere(...restOfArgs);break;
    default:log(...restOfArgs);break;
  }
  return true;
};
export const here = (silenceMsg?:AnyBoolean,...args:any[]) => !silenceMsg?print("here",...args):null;
