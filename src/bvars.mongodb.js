...
const newvars = {
  ...
  "origins" : [
      ...
      "http://localhost:4400",
      "http://localhost:4500",
      "http://192.168.1.70:4400",
      "http://192.168.1.70:4500",
      "http://192.168.1.70:3000",
      "http://10.0.0.172:4400",
      "http://10.0.0.172:4500",
      "http://10.0.0.172:3000",
      "http://172.24.191.206:4400",
      "http://172.24.191.206:4500",
      "http://172.24.191.206:3000"
  ],
  "blacklist" : [
      "123.45.67.890"
  ],
...
const createBusinessVars = async () => await db.getCollection('bvars').insertOne(newvars);
const updateBusinessVars = async (id) => {
  const _id = new mongo.ObjectId(String(id));
  const o = await db.getCollection('bvars').findOneAndUpdate({_id},updates);
