...
export class AdminService {
  ...
  static querySessions = async (
    {locQuery,...query}:PingTypes.ISessionQuery,
    select:string[],
    opts?:any,
    timestamp?:number,
  ) => {
    ...
    const results_ = await PingModels.Session.aggregate(pipeline);
    const results = locQuery?
    LocationHelpers.formatQueryResultsWithDistCalc(results_,select,locQuery):
    results_;
    Utils.log(Utils.flattenObject(results[0]));
  ...
  static queryVenues = async (
    {locQuery,...query}:PingTypes.IVenueQuery,
    select:string[],
    opts?:any,
    timestamp?:number,
  ) => {
    ...
    const results_ = await PingModels.PingVenue.aggregate(pipeline);
    const results = locQuery?
    LocationHelpers.formatQueryResultsWithDistCalc(results_,select,locQuery):
    results_;
    Utils.log(Utils.flattenObject(results[0]));
