...
const {
  ...
  IN_PROGRESS,
  CLOSED,
} = U_Types.IBugStatuses;

export class BugsService {
  // 📌 Bug CRUD Ops
  static createBugs = async (creator:string,newBugs:Partial<U_Types.IBug>[]) => {
    const bugs:U_Types.IBug[] = [];
    for(let i = 0,l = newBugs.length;i<l;i++){
      const nt = {creator,...newBugs[i]};
      const bug = new U_Models.Bug(nt);
      await bug.saveMe();
      bugs.push(bug);
    }
    return {bugs};
  };
  static createBug = async (creator:string,newBug:U_Types.IBugITO) => {
    const bug = new U_Models.Bug({creator,meta:{},...newBug});
    await bug.saveMe();
    return {bug};
  };
  static getBugById = async (bugId:string) => {
    const bug = await U_Models.Bug.findById(bugId);
  ...
  static updateBug = async (bugId:string,{notes,...updates}:any) => {
    const bug = await U_Models.Bug.findByIdAndUpdate(bugId,{
      ...notes?{$push:notes}:{},
      ...updates?{$set:updates}:{},
    },queryOpts);
    if (!bug) throw new Utils.AppError(422,'Requested bug not found');
  ...
  static deleteBug = async (bugId:string) => {
    const bug = await U_Models.Bug.findByIdAndDelete(bugId);
    if (!bug) throw new Utils.AppError(422,'Requested bug not found');
    return {ok:true};
  };
  ...
  static addFilesToBug = async (bugId:string,files:U_Types.IBug["files"]) => {
    const bug = await U_Models.Bug.findByIdAndUpdate(bugId,
      { $push: { files }},
