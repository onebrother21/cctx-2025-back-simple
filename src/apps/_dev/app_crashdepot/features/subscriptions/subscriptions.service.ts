...
export class SubscriptionsService {
  static createSubscription = async (user:Types.IUser,newSubscription:Partial<CDTypes.ISubscription>) =>  {
    const subscription = new CDModels.Subscription({creator:user.id,location:user.location,...newSubscription});
    await subscription.save();
    await subscription.populate("creator");
    return {subscription};
  };
  static getSubscriptionById = async (subscriptionId:string) => {
    const subscription = await CDModels.Subscription.findById(subscriptionId);
  ...
  static updateSubscription = async (subscriptionId:string,updates:any) => {
    const subscription = await CDModels.Subscription.findByIdAndUpdate(subscriptionId,{ $set: updates },{new:true,runValidators:true});
    if (!subscription) throw new Utils.AppError(422,'Requested subscription not found');
    await subscription.populate("creator");
    return {subscription};
  };
  static updateSubscriptionStatus = async (subscriptionId:string,status:CDTypes.ISubscriptionStatuses,info?:any) => {
    const subscription = await CDModels.Subscription.findById(subscriptionId);
    if (!subscription) throw new Utils.AppError(422,'Requested subscription not found');
  ...
  static deleteSubscription = async (subscriptionId:string) => {
    const subscription = await CDModels.Subscription.findByIdAndDelete(subscriptionId);
    if (!subscription) throw new Utils.AppError(422,'Requested subscription not found');
    return {ok:true};
  };
  static getSubscriptionsByUser = async (userId:string) => {
    const subscriptions = await CDModels.Subscription.find({ creator:userId });
  ...
  static querySubscriptions = async ({location,...query_}:any,select:string[],opts?:any,timestamp?:number) => {
    ...
    const results_ = await CDModels.Subscription.aggregate(pipeline);
