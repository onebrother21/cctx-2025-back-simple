export class PIMiaEjsController {
  static CheckLogin:IHandler = async (req,res,next) => {
    ...
    switch(true){
      ...
      case !req.session.expires:{
        res.redirect(`/pi-mia/login`);
        break;
      }
      case req.session.expires < Date.now():{
        req.session.expires = null;
        res.redirect(`/pi-mia/login?expired=true`);
        break;
      }
      default:{
        next();
  ...
  static HandleRoute:(page:string) => IHandler = (page) => (req,res) => {
    ...
    if(page == "home") res.redirect(`/pi-mia/hm`);
    else res.render('pi-mia-index',data);
  };
}
const sidebarMenu = [
  {
