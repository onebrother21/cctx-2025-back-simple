import DegenPokerModels from "../../models";
...
export class DegenPlayersQueriesService {
  // 📌 DegenPlayer Queries
  static queryDegenPlayers = async ({location,...query_}:any,select:string[],opts?:any,timestamp?:number) => {
    ...
    pipeline.push(
      { $lookup: {from: "jpmoney_profiles",localField: "creator",foreignField: "_id",as: "creator"}},
      { $unwind: "$creator"},
    ...
    // Utils.trace({query});
    let results = formatResultsWithDistCalc(await Models.Profile.aggregate(pipeline));
    return { results };
  };
