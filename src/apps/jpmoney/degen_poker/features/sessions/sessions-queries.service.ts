...
export class DegenSessionsQueriesService {
  ...
  static queryDegenSessions = async ({location,...query_}:any,select:string[],opts?:any,timestamp?:number) => {
    ...
    // Utils.trace({query});
    let results = formatResultsWithDistCalc(await UpcentricModels.DegenSession.aggregate(pipeline));
    return { results };
  };
