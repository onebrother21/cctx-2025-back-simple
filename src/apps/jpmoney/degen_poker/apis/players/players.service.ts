...
export class DegenPlayersService {
  ...
  static registerPlayer = async (req:IRequest) => {
    ...
    user.role = role;
    await AppUsage.make(`usr/${user.id}`,"createdPlayerProfile",{which:`prf/${player.id}`});
    return true;
  };
  static createDegenPlayers = async (req:IRequest) => {
    const creator:string = req.profile.id;
    const newDegenPlayers:Partial<DegenTypes.IDegenPlayer>[] = req.body.data.items;
    const players:DegenTypes.IDegenPlayer[] = [];
    for(let i = 0,l = newDegenPlayers.length;i<l;i++){
      const nt = {creator,...newDegenPlayers[i]};
      const player = new Models.Profile(nt);
      await player.saveMe();
      players.push(player as any);
    }
    return {players};
  };
